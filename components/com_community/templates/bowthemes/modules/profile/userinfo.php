...
<div class="js-focus" style="background-image:url('<?php echo $profile->cover; ?>');">
			...
			<div class="cover-block-infor-name">
				...
				<div class="cover-block-infor-detail">
					...
					<?php if( !$isMine && $config->get('enablepm')){ ?>
						<div class="btn-send-message bg-a-l">
							<a href="javascript:void(0);" class="bg-a-r userinfo-send-message" onclick="<?php echo $sendMsg; ?>">
					...
					<?php if( !$isMine ): ?>
						<?php if(!$isFriend && !$isMine && !$isBlocked): ?>
							<?php if(!$isWaitingApproval):?>
								<div class="btn-add-friend bg-a-l" onclick="joms.friends.connect('<?php echo $profile->id;?>')">
										<a class="bg-a-r userinfo-addfriend">
