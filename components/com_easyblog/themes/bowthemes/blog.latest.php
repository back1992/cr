...
<div id="ezblog-body">
	...
	<?php if( $this->getParam( 'show_headers_latestentries' , true ) ){ ?>
		<?php 
			$btBlogHead = JText::_('COM_EASYBLOG_LATEST_PAGE_HEADING');
			if($btBlogHead != "") : ?>
			<?php 
				for($i = 0; $i <= strlen($btBlogHead); $i++) {
					if(substr($btBlogHead,$i,1) == ' ') {
						$sub1btBlogHead = substr($btBlogHead,0,$i);
						$sub2btBlogHead = substr($btBlogHead,$i);
						break;
					}
				}
			?>	
	...
	<div id="ezblog-label" class="latest-post clearfix">
		<div class="bt-label-lastest-post"><span><?php echo $sub1btBlogHead; ?> </span> <?php echo $sub2btBlogHead; ?></div>
	</div>
	<?php } ?>
