...
<div id="comment-form" class="blog-section easy-blog-comment clearfix">
	<a name="commentform" id="commentform"></a>
		<?php // Tam process title span
			$titleCommentForm = JText::_('COM_EASYBLOG_LEAVE_YOUR_COMMENT');
			if($titleCommentForm != ""){
				for($i = 0; $i <= strlen($titleCommentForm); $i++) {
					if(substr($titleCommentForm,$i,1) == " ") {
						$subtitleCommentForm1 = substr($titleCommentForm,0,$i);
						$subtitleCommentForm2 = substr($titleCommentForm,$i);
						break;
					}
				}
			}
		?>
		<?php if(isset($subtitleCommentForm1) && isset($subtitleCommentForm2)){
		?>
			<h3 class="section-title" id="comment-form-title"><?php echo "<span>".$subtitleCommentForm1."</span>".$subtitleCommentForm2; ?></h3>
		<?php
		} else {
		?>
		    ...
		    <?php echo EasyBlogHelper::getHelper( 'Captcha' )->getHTML();?>
               ...
               <div class="subscription-bg"> 
					...
					<?php else : ?>
						...
						<div id="subscription-message" class="subscription-message">
							<div class="">
								...
								<label for="subscribe-to-blog"><?php echo JText::_('BT_EDUCATION_COM_EASYBLOG_SUBSCRIBE_BLOG'); ?>
								<!--<?php // if( $my->id > 0 ) : ?>
								(<?php // echo $my->email; ?>)
								<?php // else: ?>
				...
				<div class="form-row">
					...
					<label for="tnc"><?php echo JText::sprintf('BT_EDUCATION_COM_EASYBLOG_AGREE_TERMS_AND_CONDITIONS', 'javascript: ejax.load(\'entry\', \'showTnc\');'); ?></label>
				</div>
				<?php } ?>
